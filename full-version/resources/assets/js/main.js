...
const SearchConfig = {
  ...
  placeholder: document.documentElement.getAttribute('dir') === 'rtl' ? 'بحث [CTRL + K]' : 'Search [CTRL + K]',
  classNames: {
        ...
        if (data.navigation.files) {
          sources.push({
            ...
            templates: {
              header({ items, html }) {
                if (items.length === 0) return null;
                return html`<span class="search-headings">${document.documentElement.getAttribute('dir') === 'rtl' ? 'الملفات' : 'Files'}</span>`;
              },
              item({ item, html }) {
                return html`
        ...
        if (data.navigation.members) {
          sources.push({
            ...
            templates: {
              header({ items, html }) {
                if (items.length === 0) return null;
                return html`<span class="search-headings">${document.documentElement.getAttribute('dir') === 'rtl' ? 'الأعضاء' : 'Members'}</span>`;
              },
              item({ item, html }) {
                return html`
