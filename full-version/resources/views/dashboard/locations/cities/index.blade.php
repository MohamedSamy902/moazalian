...
@section('title', 'المدن - Fix-It')

@section('content')
{{-- Page Header --}}
<div class="page-header d-flex justify-content-between align-items-center">
  <div>
    <h4 class="mb-0"><i class="ti tabler-map-pin"></i> إدارة المدن</h4>
  ...
  <button type="button" class="btn btn-primary ms-auto" data-bs-toggle="modal" data-bs-target="#addCityModal">
    <i class="ti tabler-plus me-1"></i> إضافة مدينة جديدة
  </button>
</div>
  ...
  <div class="row g-3 align-items-end">
    <div class="col-md-4">
      ...
      <div class="input-group">
        ...
        <input type="text" class="form-control border-start-0 ps-0 ajax-filter" placeholder="اسم المدينة...">
      </div>
    </div>
    <div class="col-md-4">
  ...
  <div class="card-datatable table-responsive">
    <table class="table table-hover fixit-table border-top">
      <thead>
        <tr>
          <th>المدينة</th>
          <th class="text-center">عدد الأحياء</th>
          <th>الحالة</th>
      ...
      <tbody>
        ...
        @php
          $badgeClass = $city['status'] == 'active' ? 'bg-label-success' : 'bg-label-danger';
          $statusName = $city['status'] == 'active' ? 'نشط' : 'غير نشط';
        @endphp
        <tr>
          <td>
            <div class="d-flex align-items-center gap-3">
              <div class="avatar avatar-sm">
                <span class="avatar-initial rounded-circle bg-label-primary"><i class="ti tabler-building-skyscraper"></i></span>
              </div>
              <span class="fw-bold">{{ $city['name'] }}</span>
            </div>
          </td>
          <td class="text-center">
            <span class="badge bg-label-info">{{ $city['districts_count'] }} حي</span>
          </td>
          <td>
            <span class="badge {{ $badgeClass }} status-badge">
              <i class="ti {{ $city['status'] == 'active' ? 'tabler-check' : 'tabler-x' }} me-1"></i>{{ $statusName }}
            </span>
          </td>
          <td class="text-center">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title text-white"><i class="ti tabler-plus me-1"></i> إضافة مدينة جديدة</h5>
      ...
      <div class="modal-body p-4">
        <form id="addCityForm">
          <div class="mb-3">
            ...
            <input type="text" class="form-control" placeholder="مثال: الرياض" required>
          </div>
          <div class="mb-3">
            <label class="form-label fw-bold">اسم المدينة (بالإنجليزية)</label>
            <input type="text" class="form-control" placeholder="Example: Riyadh" required>
          </div>
          <div class="mb-3">
            <label class="form-label fw-bold">الحالة</label>
            <div class="form-check form-switch">
              <input class="form-check-input" type="checkbox" checked id="cityStatus">
              <label class="form-check-label" for="cityStatus">نشط</label>
            </div>
          </div>
        </form>
      </div>
      <div class="modal-footer border-top-0 p-4 pt-0">
        ...
        <button type="button" class="btn btn-primary px-4">حفظ المدينة</button>
      </div>
    </div>
  </div>
</div>
