  ...
  <ul class="navbar-nav flex-row align-items-center ms-md-auto">
    ...
    <li class="nav-item dropdown-language dropdown">
      <a class="nav-link dropdown-toggle hide-arrow btn btn-icon btn-text-secondary rounded-pill"
        href="javascript:void(0);" data-bs-toggle="dropdown" title="{{ app()->getLocale() === 'ar' ? 'العربية' : 'English' }}">
        @if(app()->getLocale() === 'ar')
          <span class="fi fi-sa" style="font-size:1.2rem;"></span>
        @else
          <span class="fi fi-gb" style="font-size:1.2rem;"></span>
        @endif
      </a>
      <ul class="dropdown-menu dropdown-menu-end">
        ...
        <li>
          <a class="dropdown-item d-flex align-items-center gap-2 {{ app()->getLocale() === $localeCode ? 'active' : '' }}"
            href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}" data-language="{{ $localeCode }}" data-text-direction="{{ $localeCode === 'ar' ? 'rtl' : 'ltr' }}">
            <span class="fi fi-{{ $localeCode === 'ar' ? 'sa' : 'gb' }}"></span>
      ...
      <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="nav-theme-text">
        <li>
          <button type="button" class="dropdown-item align-items-center active" data-bs-theme-value="light"
            aria-pressed="false">
            <span><i class="icon-base ti tabler-sun icon-22px me-3" data-icon="sun"></i>{{ __('common.light') }}</span>
          </button>
        </li>
        <li>
          <button type="button" class="dropdown-item align-items-center" data-bs-theme-value="dark" aria-pressed="true">
            <span><i class="icon-base ti tabler-moon-stars icon-22px me-3" data-icon="moon-stars"></i>{{ __('common.dark') }}</span>
          </button>
        </li>
        <li>
      ...
      <ul class="dropdown-menu dropdown-menu-end p-0">
        <li class="dropdown-menu-header border-bottom">
          <div class="dropdown-header d-flex align-items-center py-3">
            <h6 class="mb-0 me-auto">{{ __('common.notifications') }}</h6>
            <div class="d-flex align-items-center h6 mb-0">
              <span class="badge bg-label-primary me-2">{{ app()->getLocale() === 'ar' ? '٨ جديدة' : '8 New' }}</span>
              <a href="javascript:void(0)" class="dropdown-notifications-all p-2 btn btn-icon" data-bs-toggle="tooltip"
                data-bs-placement="top" title="{{ __('common.mark_all_read') }}"><i
        ...
        <li class="border-top">
          <div class="d-grid p-4">
            <a class="btn btn-primary btn-sm d-flex" href="javascript:void(0);">
              <small class="align-middle">{{ __('common.view_all_notif') }}</small>
            </a>
          </div>
        </li>
      </ul>
    ...
    @php
      ...
      $avatarUrl = $currentUser ? ($currentUser->avatar_url ?? $currentUser->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($currentUser->name).'&background=7367f0&color=fff&rounded=true') : asset('assets/img/avatars/1.png');
    @endphp
      ...
      <ul class="dropdown-menu dropdown-menu-end">
        <li>
          <a class="dropdown-item mt-0"
            @if(Auth::guard('admin')->check()) data-ajax-modal href="{{ route('admin.admins.edit', Auth::guard('admin')->id()) }}" @else href="javascript:void(0)" @endif>
            <div class="d-flex align-items-center">
              ...
              <div class="flex-grow-1">
                ...
                <small class="text-body-secondary">{{ __('common.admin') }}</small>
              </div>
            </div>
          </a>
        </li>
        ...
        <li>
          <a class="dropdown-item"
            @if(Auth::guard('admin')->check()) data-ajax-modal href="{{ route('admin.admins.edit', Auth::guard('admin')->id()) }}" @else href="javascript:void(0)" @endif>
            <i class="icon-base ti tabler-user me-3 icon-md"></i><span class="align-middle">{{ __('common.my_profile') ?? 'حسابي' }}</span> </a>
        </li>
        ...
        <li>
          <a class="dropdown-item" href="javascript:void(0)"
            ...
            <i class="icon-base bx bx-power-off icon-md me-3"></i><span>{{ __('common.logout') ?? 'تسجيل الخروج' }}</span>
          </a>
        </li>
        <form method="POST" id="logout-form" action="{{ Auth::guard('admin')->check() ? route('admin.logout') : route('logout') }}">
          @csrf
        </form>
        @else
        <li>
